...
class MlmDataset(Dataset):
    ...
    def mask_tokens(self, inputs):
        ...
        if self.config.special_tokens_mask is None:
            ...
            special_tokens_mask = self.config.special_tokens_mask.byte()
        ...
        indices_replaced = torch.bernoulli(
            torch.full(labels.shape, self.config.prob_replace_mask)).byte() & masked_indices
        inputs[indices_replaced] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)

        # 10% of the time, we replace masked input tokens with random word
        current_prob = self.config.prob_replace_rand / (1 - self.config.prob_replace_mask)
        indices_random = torch.bernoulli(
            torch.full(labels.shape, current_prob)).byte() & masked_indices & ~indices_replaced
