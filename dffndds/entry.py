...
def train(train_loader, model, epoch, optimizer, device, scheduler, print_freq=200):
    ...
    for i, batch in enumerate(train_loader):  # enumerate() 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，同时列出数据和数据下标
        ...
        ce_loss = 0.5 * (cross_entropy_loss(pre_synergy, synergyScores.squeeze(1)) + cross_entropy_loss(pre_synergy2,
        ...
        if i % print_freq == 0:
            log_str = 'TRAIN -> Epoch{epoch}: \tIter:{iter}\t Loss:{loss.val:.5f} ({loss.avg:.5f})'.format(epoch=epoch,
                                                                                                           iter=i,
                                                                                                           loss=losses)
            print(log_str)
...
def run_expriments(device):
    ...
    for split, (train_index, test_index) in enumerate(kf.split(Y)):
        trainLoader, validLoader, testLoader = define_dataloader(train_index, test_index, smiles_1, smiles_2, context,
                                                                 Y,
